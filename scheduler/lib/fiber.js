 ...
 * 超时标识
 */
Fiber.TIMEOUT_FLAG = constants.TIMEOUT_FLAG;

/**
 * 任务抛出异常，但没有具体错误信息时将得到此标识
 */
Fiber.UNKNOWN_ERR = constants.UNKNOWN_ERR;

/**
 * 队列已满标识
 */
Fiber.EXCEEDED = constants.EXCEEDED;

/**
 * 按频率调度模式
 */
Fiber.SCHEDULE_MODE_FREQUENCY = constants.SCHEDULE_MODE_FREQUENCY;

/**
 * 立即调度模式
 */
Fiber.SCHEDULE_MODE_IMMEDIATELY = constants.SCHEDULE_MODE_IMMEDIATELY;

/**
 * 饱和策略：终止
 */
Fiber.SATURATION_POLICY_ABORT = constants.SATURATION_POLICY_ABORT;

/**
 * 饱和策略：移除最早任务
 */
Fiber.SATURATION_POLICY_DISCARD_OLDEST = constants.SATURATION_POLICY_DISCARD_OLDEST;

/**
 * 任务模型，表明put进来的数据
 */
Fiber.TASK_MODE_DATA = constants.TASK_MODE_DATA;

/**
 * 任务模型，表明put进来的是函数
 */
Fiber.TASK_MODE_FUNCTION = constants.TASK_MODE_FUNCTION;

/**
 * 任务不支持，通常在往一个数据模型的队列放置函数或函数模型的队列中放置非函数时会得到此错误
 */
Fiber.TASK_ERROR = constants.TASK_ERROR;

/**
 * 任务被取消（在饱和策略设置为丢弃最早任务时才会出现）
 */
Fiber.DISCARD = constants.DISCARD;

/**
 * 提交了一个空数组时报错
 */
Fiber.EMPTY_ELEMENTS = constants.EMPTY_ELEMENTS;
