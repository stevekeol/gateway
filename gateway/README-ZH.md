...
2. 希望任务可以在失败的时候重试。相比于自行实现，fiber可以提供更加多样且易于使用的重试策略。

3. 希望任务可以提供超时取消的能力，也可以提供每个任务的执行指标，包括在队列中的等待时长，运行时长，执行状态等。

4. 希望可以控制异步任务的总量，在达到一个阈值之后，fiber会采取拒绝或丢弃旧任务等方式避免任务出现不可控的堆积现象。

总结：可以将 `fiber` 理解为一个线程池，所有异步任务都可以通过 `fiber` 来调度，有了 `fiber` 之后，异步任务不再是野蛮堆积的，而是完全可控的。
...
```
const Fiber = require('@bfs/fiber');

// 新建队列实例
const queue = new Fiber();
...
queue.put([]).catch(err => {
    assert.equal(err, Fiber.EMPTY_ELEMENTS);
});
```
...
// 新建队列实例，启用按频率调度模式
const queue = new Fiber({
    // 常量直接挂载在Fiber下，设定会按频率调度
    scheduleMode: Fiber.SCHEDULE_MODE_FREQUENCY,
    // 设定每秒执行两个任务，fiber会计算出每500ms执行一个任务
    countPerTimeScale: 2
});
...
queue.push([task(), task()， task()]).then(ts => {
    ...
    assert.equal(ts.res[2].err, Fiber.DISCARD);
});
...
| scheduleMode | 调度模式，支持按频率调度和即时调度 | Fiber.SCHEDULE_MODE_FREQUENCY和Fiber.SCHEDULE_MODE_IMMEDIATELY（默认），运行期不支持更改 |
| countPerTimeScale | 按频率调度时，每个时间刻度调度的任务数 | 大于0，默认为100，设置为-1时将每次调度所有任务，运行期不支持更改 |
| timeScale | 时间刻度（s） | 大于0，默认为1
| taskMode | 任务模式，即put时，任务的类型属于函数还是数据 | Fiber.TASK_MODE_DATA和Fiber.TASK_MODE_FUNCTION，没有设置的话将自动推断，运行期不支持更改 |
| handler | 任务为数据模式时，需要提供handler函数 | 执行返回Promise的函数 |
| saturationPolicy | 队列饱和时提交任务的策略 | Fiber.SATURATION_POLICY_ABORT（终止，默认）和Fiber.SATURATION_POLICY_DISCARD_OLDEST（丢弃最早任务） |
...
Fiber挂载了一些字符串常量，例如任务取消执行的标识：Fiber.DISCARD，可优先考虑使用。

> Fiber.prototype.put(tasks, prior)

别名函数：Fiber.prototype.push(tasks, prior)。
...
> Fiber.prototype.setOption(options)

更新队列实例的配置，除运行期不支持更改的配置项外，其它配置项都可以随意更新，即时生效。

> Fiber.prototype.runningTasksCount

获取运行中的任务数。

> Fiber.prototype.queueTaskSize

获取队列中等待的任务数。

> Fiber.prototype.options

获取当前实例的配置。

> Fiber.prototype.destory()
